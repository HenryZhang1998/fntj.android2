 ...
 *   </pre>
 */
public abstract class BaseRecyclerActivity<T, VH extends RecyclerView.ViewHolder, A extends RecyclerView.Adapter> extends BaseActivity implements OnItemClickListener, OnItemLongClickListener {
    ...
    public synchronized void saveCache(List<T> newList) {
        ...
        CacheManager.getInstance().saveList(cacheCallBack.getCacheClass(), cacheCallBack.getCacheGroup(), map, saveCacheStart, cacheCallBack.getCacheCount());
    }
    //缓存>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

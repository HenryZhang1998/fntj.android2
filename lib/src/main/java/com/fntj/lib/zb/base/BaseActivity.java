...
import com.fntj.lib.zb.interfaces.ActivityPresenter;
import com.fntj.lib.zb.interfaces.OnBottomDragListener;
import com.fntj.lib.zb.manager.SystemBarTintManager;
import com.fntj.lib.zb.manager.ThreadManager;
import com.fntj.lib.zb.util.ScreenUtil;
...
//	}
    ...
    public void initTitle(String str, int tv_title, int btn_back) {
        TextView textView = findViewById(tv_title);
        if (textView != null) {
            textView.setText(str);
        }
        View findViewById = findViewById(btn_back);
        if (findViewById != null) {
            findViewById.setOnClickListener(new OnClickListener() {
                public void onClick(View view) {
                    BaseActivity.this.finish();
    ...
    protected void hideSoftInputWindow(Context context) {
        InputMethodManager imm = (InputMethodManager) context.getSystemService(Context.INPUT_METHOD_SERVICE);
        if(isInputMethodShowing()) {
            LogUtils.i("hideSoftInputWindow:");
            imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
        }
    }
    protected void showSoftInputWindow(Context context) {
        InputMethodManager imm = (InputMethodManager) context.getSystemService(Context.INPUT_METHOD_SERVICE);
        if(!isInputMethodShowing()) {
            LogUtils.i("hideSoftInputWindow:");
            imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
